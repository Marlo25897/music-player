...
class View:
    def __init__(self, top=None):
        ...
        self.songName.configure(font=font12)
    ...
    def setup_player(self):
        try:
            ...
            self.Button9.configure(state="disable")
            self.Button11.configure(state="disable")
            self.Button10.configure(state="disable")
        ...
        self.playList.configure(font='Gabriola 12')
    ...
    def change_volume(self,x):
        x=self.vol_scale.get()
        self.my_player.set_volume(x/100)
    ...
    def add_song(self):
        ...
        rcolor=lambda :random.randint(0,150)
    ...
    def show_song_details(self):
        ...
        min=round(min)  #for float to int
        sec=round(sec)
    ...
    def play_song(self):
        ...
        try:
            ...
            self.song_name=self.playList.get(self.sel_song_index_tuple[0]) #will return String because one item in tuple
    ...
    def list_double_click(self,e):  #event e because handle by bind not command
        self.play_song()
    ...
    def remove_song_from_favourites(self):
        rem_song_index_tuple=self.playList.curselection()
        try:
            if len(rem_song_index_tuple)==0:
                raise NoSongSelectedError("Please select a song to remove from favourites")
            song_name=self.playList.get(rem_song_index_tuple[0])
            result=self.my_player.remove_song_from_favourites(song_name)
            if result.find("not present")!=-1:
                messagebox.showinfo("Error!!",result)
            if result.find("deleted")!=-1:
                self.playList.delete(rem_song_index_tuple[0])
                if self.sel_song_index_tuple[0]==rem_song_index_tuple[0]:
                    self.stop_song()
                messagebox.showinfo("Success!!",result)
        except(NoSongSelectedError) as ex1:
            messagebox.showerror("Error!!", ex1)
    ...
    def load_songs_from_favourites(self):
        try:
            ...
            rcolor=lambda :random.randint(0,200)
            #rcolor=lambda :random.randint(0,255)
            red=hex(rcolor())
            green=hex(rcolor())
            blue=hex(rcolor())
